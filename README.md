...
| 变量 | 默认值 | 说明 |
|------|--------|------|
| `ASR_MODEL_PATH` | `./models/Qwen3-ASR-1.7B` | 填写你的模型存放地址，默认放在该项目的models文件夹下 |
| `GPU_MEMORY_UTILIZATION` | `0.4` | vLLM 显存占用（相对单卡总显存） |
| `MAX_MODEL_LEN` | `4000` | vLLM 最大上下文；模型默认 65536 会占用大量 KV cache |
| `MAX_NEW_TOKENS` | `64` | 流式 max tokens |
| `HF_HUB_OFFLINE` | `true` | 为 `true` 时仅从本地缓存加载模型，不访问网络 |
| `HOST` | `0.0.0.0` | 监听地址 |
| `PORT` | `9800` | 监听端口 |
...
通过 **HTTP + IP 地址**（如 `http://196.168.30.200:9800`）打开 `demo.html` 时会出现此错误。浏览器要求麦克风 API 运行在安全上下文中。
